...
@Injectable()
export class ExtendsUploadService {
  ...
  constructor(
    ...
    this.thunkDir = 'thunk'
    this.isLocal = this.config.get('extends.file.isLocal')
  }
   ...
   * @returns
   */
  async singleFileUpload(file: Express.Multer.File, user: MemberUserType, module?: string, feature?: string) {
    const fileSize = (file.size / 1024 / 1024).toFixed(2)
    if (fileSize > this.config.get('extends.file.maxSize')) {
      return ResultData.fail(500, `文件大小不能超过${this.config.get('extends.file.maxSize')}MB`)
    }
    const uploadId = GenerateUUID()
    const ext = this.getExt(file)
    const newFileName = `${uploadId}.${ext}`
    const targetPath = path.join(module || '', feature || '')

    let res
    if (this.isLocal) {
      res = await this.saveFileLocal(targetPath, newFileName, file)
    }
    else {
   ...
   * @param file
   */
  async saveFileLocal(targetPath: string, newFileName: string, file: Express.Multer.File) {
    const rootPath = process.cwd()
    // 文件根目录
    const baseDirPath = path.join(rootPath, this.config.get('extends.file.location'))
