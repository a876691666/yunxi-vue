...
import { isString, isNull, isEmpty } from '@/utils/is';
...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    // 说实话操蛋的很 data xxx xx全部返回  不能统一吗?
    const { code, msg, data, ...other } = axiosResponseData;
    ...
    if (hasSuccess) {
      ...
      // ruoyi没有采用严格的{code, msg, data}模式
      // 如果有data (需要考虑data是否还有其他参数) 没有data将剩余参数(...other)封装为data返回
      // 只包含data的情况
      if (data && Object.keys(axiosResponseData).length === 3) {
        return data;
      }
      // 包含data 还有其它参数
      if (data) {
    ...
    deepMerge(
      {
        ...
        /**
         * 序列化设置 否则无法传递 Map<String, xxx>类型参数
         * 比如日期区间
         * @param params
         * @returns
         */
