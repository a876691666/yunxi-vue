...
import type { GroupForm } from '#/api/member/group/model';
...
function handleDownloadExcel() {
  commonDownloadExcel(groupExport, '用户分组表数据', tableApi.formApi.form.values, {
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="用户分组表列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['member:group:export']"
          ...
          <a-button
            ...
            danger
            type="primary" 
            v-access:code="['member:group:remove']" 
            @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </a-button>
          <a-button
            type="primary"
            v-access:code="['member:group:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button
            v-access:code="['member:group:edit']"
    ...
    <!-- <GroupDrawer @reload="tableApi.query()" /> -->
  </Page>
</template>
